...
for (let i = 0; i < children.length; i++) {
  ...
  child.addEventListener('click', () => {
    ...
    for (let j = 0; j < children.length; j++) {
      ...
      children[j].style.top = `calc(var(--oval-height) / 2 - var(--child-size) / 2 - (var(--oval-height) / 2) * sin(${newChildAngle}))`;
      children[j].style.left = `calc(var(--oval-width) / 2 - var(--child-size) / 2 + (var(--oval-width) / 2) * cos(${newChildAngle}))`;
